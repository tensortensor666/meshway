...
需要 Rust stable、Node.js 22+。复制 `.env.example` 为 `.env`，至少修改 `MESHWAY_ADMIN_TOKEN`，然后由 CI 或本地环境构建 `meshway` 二进制。运行后访问 `http://127.0.0.1:8080/health`。
...
`.github/workflows/build.yml` 在 pull request、`main`/`master` push 和 `v*` tag 上运行。构建由 GitHub Actions 完成并上传：
